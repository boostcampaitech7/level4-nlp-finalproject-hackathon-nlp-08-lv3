...
from common import load_all_book_chunks
...
if platform.system() == "Linux":
    font_path_regular = "/usr/share/fonts/truetype/nanum/NanumGothic.ttf"  # 나눔고딕 일반체
    font_path_bold = "/usr/share/fonts/truetype/nanum/NanumGothicBold.ttf"  # 나눔고딕 볼드체
else:
...
def draw_logo(c, width, height):
    ...
    try:
        ...
        c.drawImage(ImageReader(logo_path), width-80, height-810, width=40, height=40)
    except Exception as e:
...
def draw_profile_box(c, data, width, height):
    ...
    grade_paragraph.drawOn(c, line_x_start2 + 70, grade_y)  # 등급(S, A, B, C, D) 위치 지정

# ==================================  # 표, 막대그래프
def draw_table(c, data, width, height):
    table_data = [
        ["평가항목", "점수 (5점 만점)"], *data['scores'],
        ["합계", f"{data['total_score']:.2f}"]
    ]
    
    table = Table(table_data, colWidths=[115, 115]) # 열 너비 통일
    table.setStyle(TableStyle([
        ('BACKGROUND', (0,0), (-1,0), colors.HexColor("#08c7b4")), # 헤더 배경색 (민트색)
...
def draw_assessment_box(c, data, width, height):
    ...
    c.rect(width, height, box_width, box_height, fill=1, stroke=0)  # 회색 박스 위치, 크기, 채우기, 테두리 없음
